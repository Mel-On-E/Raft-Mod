...
Axe.swingCount = 2
Axe.mayaFrameDuration = 1.0/30.0
Axe.freezeDuration = 0.075

Axe.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
Axe.swingFrames = { 4.2 * Axe.mayaFrameDuration, 4.2 * Axe.mayaFrameDuration }
Axe.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }

function Axe.client_onCreate( self )
	self.isLocal = self.tool:isLocal()
	self:init()
end

function Axe.client_onRefresh( self )
	self:init()
	self:loadAnimations()
end

function Axe.init( self )
...
function Axe.loadAnimations( self )
...
function Axe.client_onUpdate( self, dt )
...
function Axe.updateFreezeFrame( self, state, dt )
...
function Axe.server_startEvent( self, params )
...
function Axe.client_startLocalEvent( self, params )
	self:client_handleEvent( params )
end

function Axe.client_handleEvent( self, params )
...
--function Axe.sv_n_toggleTumble( self )
--	local character = self.tool:getOwner().character
--	character:setTumbling( not character:isTumbling() )
--end

function Axe.client_onEquippedUpdate( self, primaryState, secondaryState )
...
function Axe.client_onEquip( self, animate )
...
function Axe.client_onUnequip( self, animate )
...
function Axe.sv_updateBlocking( self, isBlocking )
