...
local function UpdateCharacterInWater( trigger, character )
	...
	--Raft
	--had to change this because it would set the speed fraction back to 1 when the player had the fins
	--took a while to notice this

	-- Scaled movement slowdown when walking through water
	local isPlayer = character:isPlayer()
	local player
	local inv
	if isPlayer then
		player = character:getPlayer()
		inv = sm.game.getLimitedInventory() and player:getInventory() or player:getHotbar()
	end

	if not isPlayer or not sm.container.canSpend( inv, obj_fins, 1 ) then
		if not character:isSwimming() then
